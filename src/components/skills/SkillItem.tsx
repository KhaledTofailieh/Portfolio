  ...
  return (
    <div className="flex items-center justify-between py-1.5 px-2 rounded-md bg-secondary/30 hover:bg-primary/10 transition-colors duration-200">
      <div className="flex items-center gap-2 min-w-0 flex-1">
        <div className="w-[30px] h-[30px] bg-amber-50 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden">
          <TechnologyIcon
            src={skill.icon || getFallbackIcon()}
            alt={`${skill.name} icon`}
            size={22}
            className={skill.icon?.includes('lobehub') ? 'scale-[1.5]' : ''}
          />
        </div>
        <div className="flex items-center gap-2 min-w-0 flex-1">
