...
const PublicationCategoryFilter: React.FC<PublicationCategoryFilterProps> = ({ publications }) => {
  ...
  const counts: Record<Category, number> = {
    ...
    research: publications.filter((p) => p.category === 'research').length,
    blog: publications.filter((p) => p.category === 'blog').length,
  };
          ...
          {categories.map((category) => {
            ...
            return (
              <button
                ...
                className={`px-6 py-2 rounded-full text-sm font-medium transition-all duration-200 flex items-center gap-1.5 ${
                  isSelected
              ...
              >
                ...
                {isSelected && (
                  <span className="inline-flex items-center justify-center min-w-5 h-5 px-1.5 text-xs font-bold rounded-full bg-accent-foreground/20 text-accent-foreground animate-fade-in">
                    {counts[category.id]}
                  </span>
                )}
              </button>
